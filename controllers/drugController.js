...
exports.getDrugByDrugbankId = async (req, res, next) => {
	try {
		const drug = await models.Drug.findOne({ 'drugbank-id': req.params.id });
		success(res, drug);
	} catch (err) {
		next(err);
	}
...
exports.getInteractionBetweenTwoDrugs = async (req, res, next) => {
	try {
		const drug = await models.Drug.findOne({ 'drugbank-id': req.params.drugId1 });
		if (!drug) {
		...
		const interaction = interactions.find((interaction) => interaction['drugbank-id'] === req.params.drugId2);

		if (!interaction) {
			return success(res, { message: 'No interaction found between these drugs' });
		}
